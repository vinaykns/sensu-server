...
class SNMPIfStats < Sensu::Plugin::Metric::CLI::Graphite
  ...
  def run
	...
	if config[:threshold].to_f > 0 && ifInSpeed > config[:threshold].to_f
	   mutated.push("In Speed")
	   mutated.push(ifInSpeed)
	   mutated.push(switchName)
	   mutated.push(interfaceName)
	   if interfaceName.include? "ethernet1"
           	critical_state = true
	   end	
	else
	   normal.push("In Speed")
	   normal.push(ifInSpeed)
           normal.push(switchName)
           normal.push(interfaceName)
	end
...
if config[:threshold].to_f > 0 && ifOutSpeed.to_f > config[:threshold].to_f
	    mutated.push("Out Speed")
            mutated.push(ifOutSpeed)
            mutated.push(switchName)
            mutated.push(interfaceName)
	    if interfaceName.include? "ethernet1"
            	critical_state = true   
	    ...
	    normal.push("Out Speed")
            normal.push(ifOutSpeed)
            normal.push(switchName)
            normal.push(interfaceName)	
	end
    ...
    if true == critical_state
     target = open('/etc/sensu/plugins/mutated.txt','w')
     target.write(mutated)
     target.write("\n")
     target.close
     file = open('/etc/sensu/plugins/normal.txt','w')
     file.write(normal)
     file.write("\n")
     file.close
     critical

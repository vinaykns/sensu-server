...
class CheckSensor < Sensu::Plugin::Metric::CLI::Graphite
  ...
  def run
    # #YELLOW
    res= `ipmitool -I lanplus -U #{config[:username]} -P #{config[:password]} -H #{config[:host]} -S /etc/sensu/plugins/sdr_dump_file sensor reading FAN1_TACH1 FAN1_TACH2 FAN2_TACH1 FAN2_TACH2 FAN3_TACH1 FAN3_TACH2 FAN4_TACH1 FAN4_TACH2 FAN5_TACH1 FAN5_TACH2 P1_TEMP_SENS P2_TEMP_SENS DDR3_P1_A1_TEMP DDR3_P1_B1_TEMP DDR3_P1_C1_TEMP DDR3_P1_D1_TEMP DDR3_P2_E1_TEMP DDR3_P2_F1_TEMP DDR3_P2_G1_TEMP DDR3_P2_H1_TEMP POWER_USAGE PCH_TEMP_SENS`

    res.each_line do |line|
      sensor = line.split

      name = sensor[0].downcase
      value = sensor[2]

      output "#{config[:scheme]}.#{name}", value
    end
    ok
  end
end
